...
function nextNewYear() {
  ...
  const diff = nextNewYearDate - currentDate;
...
function getAge(birthDate) {
  ...
  const birthDateISOString = birthDate.toISOString();
...
function compareDates(date1, date2) {
  ...
  const comparisonResult = date1.getTime() - date2.getTime();
  // "Помилка: вхідне значення має бути об'єктом Date"
  ...
  const date1ISOString = date1.toISOString();
  const date2ISOString = date2.toISOString();
  // Повертаємо об'єкт з порівнюваними датами та результатом порівняння.
  return {
    date1: date1ISOString,
    date2: date2ISOString,
    comparison: Math.sign(comparisonResult),
  };
}
